 ...
 ******************************************************************************/
void app_log_init();
sl_status_t sl_iostream_printf(const char *format, ...);
...
#define app_log_append(...)                          \
  ...
  sl_iostream_printf(__VA_ARGS__); \
...
#define app_log_print_trace()              \
  sl_iostream_printf(APP_LOG_TRACE_FORMAT, \
                     __FILE__,             \
                     __LINE__,             \
                     __func__)
...
#define app_log_hexdump_level_s(level, separator, p_data, len) \
  do {                                                         \
    if (app_log_check_level(level)) {                          \
      ...
      app_log_append(APP_LOG_NEW_LINE_RN);                     \
    }                                                          \
  } while (0)
