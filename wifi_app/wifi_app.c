...
static sl_status_t show_scan_results()
{
  ...
  if (scan_result->scan_count) {
    ...
    for (int a = 0; a < (int)scan_result->scan_count; ++a) {
      ...
      app_log_info("  %02x:%02x:%02x:%02x:%02x:%02x, %4u,  -%u\r\n",
                bssid[0],
                bssid[1],
                bssid[2],
                bssid[3],
...
sl_status_t wifi_client_connect(char *ssid, char *password, sl_wifi_security_t security_type)
{
  ...
  app_log_info("PWD=%s\r\n", password);
  status = sl_wifi_connect(SL_WIFI_CLIENT_2_4GHZ_INTERFACE, &client_access_point, TIMEOUT_MS);

  if (status != RSI_SUCCESS) {
    app_log_error("WLAN Connect Failed, Error Code : 0x%lX\r\n", status);
    catcollar_wifi_connection_set_state(CATCOLLAR_WIFI_DISCONNECTED);
  } else {
    printf("\n WLAN connection is successful!\r\n");
...
{
  sl_status_t status;
  sl_net_ip_configuration_t dhcp_ip_address  = { 0 };

  status = wifi_client_connect(test_access_point_info.ssid, test_access_point_info.pwd, SL_WIFI_WPA2);
  if (status != SL_STATUS_OK) {
    app_log_error("Failed to connect to test access point: 0x%lx\r\n", status);
  } else {
    dhcp_ip_address.type      = SL_IPV4;
    dhcp_ip_address.mode      = SL_IP_MANAGEMENT_DHCP;
    dhcp_ip_address.host_name = DHCP_HOST_NAME;
    // Configure IP
    status = sl_si91x_configure_ip_address(&dhcp_ip_address, SL_SI91X_WIFI_CLIENT_VAP_ID);
    if (status != RSI_SUCCESS) {
      catcollar_wifi_connection_set_state(CATCOLLAR_WIFI_DISCONNECTED); 
    } else {
      sl_ip_address_t ip = { 0 };
...
static catcollar_wifi_connection_state_t catcollar_wifi_connection_state = 0;
int catcollar_wifi_connection_get_state(void)
{
  return catcollar_wifi_connection_state;
}

void catcollar_wifi_connection_set_state(catcollar_wifi_connection_state_t state)
{
  catcollar_wifi_connection_state = state;
  if (state == CATCOLLAR_WIFI_DISCONNECTED) {
    leds_play(GREEN_LED, LEDS_SLOW_BLINK);
  } else if (state == CATCOLLAR_WIFI_CONNECTED) {
