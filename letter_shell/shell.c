...
void shellInit(Shell *shell, char *buffer, unsigned short size)
{
    ...
    // 创建二值信号量
    // shell->rxSemaphore = xSemaphoreCreateBinary();
    // shell->rxQueue = xQueueCreate(16, sizeof(char));
}
...
void shellTask(void *param)
{
    ...
    while(1)
    {
        ...
        // }
        // if (xSemaphoreTake(shell->rxSemaphore, portMAX_DELAY) == pdTRUE)
        // {
            // while (xQueueReceive(shell->rxQueue, &data, 0) == pdTRUE)
            // {
            //     shellHandler(shell, data);
            // }
            if (shell->read && shell->read(&data, 1) == 1)
            ...
            // while (console_data_rx_receive)
            // {
            //     if (console_read_data_from_cache(&data, 1) == 1)
            //     {
            //         shellHandler(shell, data);
            //     }
            // }
