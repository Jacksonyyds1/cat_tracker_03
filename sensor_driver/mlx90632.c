...
sl_status_t mlx90632_init(void)
{
  ...
  ret = platform_read16(2, MLX90632_ADDR, NULL, MLX90632_EE_PRODUCT_CODE, &product_code, 2);
  if (ret < 0){
    sc = SL_STATUS_FAIL;
  }
  app_log_debug("MLX90632->CHIP ID->0x%x\r\n", product_code);

  if (product_code != MLX90632_PRODUCT_CODE) {
    return NOT_MATCHING_PRODUCT_CODE;
  }
